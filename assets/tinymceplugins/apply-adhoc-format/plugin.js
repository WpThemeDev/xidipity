...
tinymce.PluginManager.add('app_adhoc_fmt', function (editor) {
	...
	function frmInit() {
		...
		switch (true) {
			...
			case (oDoc.lineCnt > 1):
				// multi line
				htmlDisplay = regDecode(oDoc.blkContent());
				break;
			case (oDoc.isFragment()):
				// content fragment
				switch (true) {
					case (isEmpty(oDoc.fullHtml().match('>' + oDoc.mceHtml() + '<'))):
						htmlDisplay = regDecode(oDoc.mceHtml());
						break;
					case (oDoc.mceNodeName() == 'span'):
						htmlDisplay = regDecode(oDoc.proMceTags(oDoc.mceNode.parentNode.outerHTML));
						break;
					case (oDoc.hasMark):
						htmlDisplay = regDecode(oDoc.outerHtml());
						break;
					default:
						htmlDisplay = regDecode(oDoc.mceHtml());
				}
			...
			case (oDoc.hasMark):
				// contains <i>, <u>, etc.
				htmlDisplay = regDecode(oDoc.fullHtml());
				break;
	...
	function frmSubmit() {
		...
		try {
			...
			if (oDoc.lineCnt == 1) {
				...
				//
				console.log('*** * blkArry #:  ' + blkArray.length);
				//
				var blkNode;
				var blkHtmlItem;
				...
				for (; blkArray[idx];) {
					...
					//
					console.log('*** * blkHtmlItem: ' + blkHtmlItem);
					//
					switch (true) {
						case (_err.hasError()):
							// error
	...
	editor.addButton('app_adhoc_fmt', {
		title: 'Ad Hoc Format',
		icon: false,
		image: 'data:image/svg+xml;base64,',
		onclick: function () {
			if (!mceReady()) {
				_err.display();
				editor.focus();
				...
				if (!_err.hasError()) {
					...
					if (oDoc.lineCnt == 1 && !isEmpty(oDoc.fullHtml().match(/class/)) && isEmpty(oDoc.fullHtml().match(/style/))) {
					...
					var mceTag = getRegExpValue(oDoc.fullHtml(), '(^<(p|h[1-6]|div|li|td|th).*?>)(<span.*?>)|(^<(p|h[1-6]|div|li|td|th).*?>)', 's');
					var optOne;
					document.getElementById('attr_id').selectedIndex = '0';
					switch (true) {
						case (oDoc.isFragment() && isEmpty(oDoc.outerHtml().match(/<span.*?>/g))):
							//
							console.log('    - Mark -');
							//
							if (frmActionCnt > 1) {
							...
							break;
						case (oDoc.lineCnt > 1 && isEmpty(oDoc.mceHtml().match(/(class|style).*?"(.*?)"/gm))):
							//
							console.log('    - Mark -');
							//
							if (frmActionCnt > 1) {
							...
							document.getElementById('act_id')[0].text = 'Apply';
							break;
						case (oDoc.lineCnt == 1 && isEmpty(mceTag.match(/(class|style).*?"(.*?)"/g))):
							//
							console.log('    - Mark -');
							//
							if (frmActionCnt > 1) {
							...
							document.getElementById('act_id')[0].text = 'Apply';
							break;
						case (oDoc.isFragment() && isEmpty(oDoc.fullHtml().match('>' + oDoc.mceHtml() + '<'))):
							//
							console.log('    - Mark -');
							//
							if (frmActionCnt > 1) {
...
/*
 * EOF: apply-adhoc-format / plugin.js / 210704-1
 */
