/*
 *  Xidipity WordPress Theme
 *
 *  file:   twocolumn/plugin.js
 *  build:  91025.1a
 *  descrp: two column plugin
 *  ref:    https://www.tiny.cloud/
 *
 *  @package WordPress
 *  @subpackage Xidipity
 *  @since 0.9.0
 *
 **/
tinymce.PluginManager.add('twocolumn', function (editor, url) {
    editor.addButton('twocolumn', {
        title: 'Insert 2 column template',
        icon: 'mce-column',
        image: 'data:image/svg+xml;base64,',
        onclick: function () {
            var dom = editor.dom;
            var uniqueID = dom.uniqueId();
            var html = '<!-- xwpt: 90927.1a/mce/toolbar/multi-col   --><table class="cols:auto"><tbody><tr><td><p id="' + uniqueID + '">&nbsp;</p></td></tr></tbody></table><!-- /xwpt: 90927.1a/mce/toolbar/multi-col  -->';
            editor.insertContent(html);
            var newTwoColumn = dom.select('p#' + uniqueID)[0];
            editor.selection.setCursorLocation(newTwoColumn);
        }
    });
});
/*
    eof: twocolumn/plugin.js
*/
